...
public class RefreshLayout extends ViewGroup {
    ...
    private ValueAnimator mRefreshValueAnimator;
    private int mFrom = 0;
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        if (!isEnabled() || mReturningToStart || canChildScrollUp()) {
            // Fail fast if we're not in addRefreshView state where addRefreshView swipe is possible
            return false;
        }

        switch (action) {
            case MotionEvent.ACTION_DOWN:
                setTargetOffsetTopAndBottom(mIndicator.getOriginalOffsetTop() - mHeader.getTop(),
                        true);
    ...
    private void onSecondaryPointerUp(MotionEvent ev) {
        ...
        if (pointerId == mActivePointerId) {
            // This was our active pointer going up. Choose addRefreshView new
    ...
    private void moveSpinner(float overScrollTop) {
        ...
        int targetY =
                mIndicator.getOriginalOffsetTop() + (int) ((slingshotDist * dragPercent) +
                        extraMove);
        setTargetOffsetTopAndBottom(targetY - mIndicator.getCurrentOffsetTop(), true);
    }

    private void finishSpinner(float overScrollTop) {
        if (overScrollTop > 100000) {

        } else {
            mIndicator.setCurrentOffsetTop(-1);
            mFrom = mHeader.getTop();
    ...
    public void setHeader(View header) {
        if (!allowDesire(header, mHeader)) {
            return;
        }

        mHeader = addRefreshView(header, mHeader);
    }

    public void setFooter(View footer) {
        if (!allowDesire(footer, mFooter)) {
            return;
        }

        mFooter = addRefreshView(footer, mFooter);
    }

    private boolean allowDesire(View desireView, View existView) {
        return !(desireView == null || desireView == existView);
    }

    private View addRefreshView(View view, View existView) {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        if (attrs != null) {
            ...
            mIndicator.setDuration(arr.getInt(R.styleable.RefreshLayout_duration,
                    mIndicator.getDuration()));
        ...
        mRefreshValueAnimator = ValueAnimator.ofInt(1, 100);
        mRefreshValueAnimator.setDuration(300);
        mRefreshValueAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            @Override
            public void onAnimationUpdate(ValueAnimator animation) {
                float value = animation.getAnimatedFraction();

                int targetTop = (mFrom + (int) ((mIndicator.getOriginalOffsetTop() - mFrom) *
                        value));
                int offset = targetTop - mHeader.getTop();
                D("Value->" + value + " Top->" + targetTop + " Offset->" + offset);
